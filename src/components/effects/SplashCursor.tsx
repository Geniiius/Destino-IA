/**
 * @file components/effects/SplashCursor.tsx
 * @description Effet de curseur fluide WebGL pour pages d'accueil
 */
...
export default function SplashCursor({
  ...
  useEffect(() => {
    ...
    function generateColor(): ColorRGB {
      // Palette de couleurs vibrantes: vert, cyan, violet, rose, orange
      const colorPalettes = [
        { h: 0.45, label: "emerald" }, // Verde esmeralda
        { h: 0.5, label: "cyan" }, // Cyan
        { h: 0.55, label: "teal" }, // Verde azulado
        { h: 0.75, label: "purple" }, // Púrpura
        { h: 0.9, label: "magenta" }, // Magenta
        { h: 0.05, label: "orange" }, // Naranja
        { h: 0.15, label: "yellow" }, // Amarillo
      ];

      const palette =
        colorPalettes[Math.floor(Math.random() * colorPalettes.length)]!;
      const h = palette.h + (Math.random() - 0.5) * 0.08;
      const s = 0.85 + Math.random() * 0.15; // Saturación alta
      const v = 0.55 + Math.random() * 0.1; // Brillo moderado

      const c = HSVtoRGB(h, s, v);

      // Intensidad moderada para éviter trop de blanc
      c.r *= 0.35;
      c.g *= 0.35;
      c.b *= 0.35;
